...
const usd = (mt = 'real\\Broker\\STP'): CrmPnlAccount => ({ centsFactor: 1, metaTraderGroup: mt });
const cent = (mt = 'real\\Cent\\STP'): CrmPnlAccount => ({ centsFactor: 100, metaTraderGroup: mt });
...
describe('aggregateCrmDailyPnl — lo excluido se cuenta', () => {
  ...
  it('un factor cero o negativo se trata como 1 y no revienta la división', () => {
    const { days } = aggregateCrmDailyPnl(
      ...
      new Map([['1', { centsFactor: 0, metaTraderGroup: 'real\\Broker\\STP' }]]),
    );
