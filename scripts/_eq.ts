...
async function main(){
  ...
  await withMt5Connection('71715987-5479-52c4-a990-c414fb3a9b36', async (s) => {
    const c = await s.query('SHOW COLUMNS FROM mt5_users');
    console.log('  mt5_users tiene Equity?', c.some((x)=>x.Field==='Equity'));
    const a = await s.query('SHOW COLUMNS FROM mt5_accounts');
    console.log('  mt5_accounts:', a.map((x)=>x.Field).join(', '));
    const t0=Date.now();
